...
TEMP_PASSWORD=$(docker logs homeserver-qbittorrent 2>&1 | grep "temporary password" | tail -1 | sed 's/.*: //')
...
curl -c /tmp/cookies.txt \
  ...
  -X POST http://localhost:8081/api/v2/auth/login

curl -b /tmp/cookies.txt \
  -H "Referer: http://localhost:8081" \
  -d 'json={"bypass_auth_subnet_whitelist_enabled":true,"bypass_auth_subnet_whitelist":"0.0.0.0/0"}' \
  -X POST http://localhost:8081/api/v2/app/setPreferences

sleep 3
VERSION=$(curl -s http://localhost:8081/api/v2/app/version)
